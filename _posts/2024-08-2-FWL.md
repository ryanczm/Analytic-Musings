...
So _in theory_, when you assess a single beta in a multiple regression, you don't need to worry about it's correlation to other features because it's correlation has been partialled out! _In practice_, this obviously is not the case, but that's why ridge/SVD is used.
...
For now, the FWL theorem provides us with essential intuition about key facets of multiple regression without going into any implementation details yet. Having discovered it two weeks ago while going through the regression part of ESL and EQI, it's nice that Gundersen and RyxCommar have very good online resources to explain the theorem.
