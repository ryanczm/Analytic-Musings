...
category: regression
excerpt: "In this article, I take a look at the Frisch-Waugh-Lovell (FWL) theorem, which lets us understand the interpretation of individual betas in a multiple regressioon after partialling out correlation effects, and lets us view the intercept as orthogonalizing the features with regard to a constant vector. Featuring stuff from Gregory Gundersen, RyxCommar."
---
...
The nature of the work involves quantifying relationships between noisy, high-dim, collinear, non-stationary data, and regression is the tool of choice. The FWL theorem lets us spruce up our interpretation of regression in a few ways that are useful.
...
So what is it anyway? The theorem can be motivated by this question:
...
The FWL theorem formalizes this notion. It states the individual beta of a feature in a multiple regression is a partial effect: the effect of that variable after residualising/orthogonalising out the influence of the other predictors. For example, if we have 

$$Y=\beta_1X_1+ \beta_2X_2+\epsilon$$ 

and want to isolate $\beta_1$, we can first regress $y \sim X_2$ and $X_1 \sim X_2$ and take the residuals, then regress first set of residuals on the second set. FWL theorem tells us the slope of this regression is our isolated beta $\beta_1$: 
...
So _in theory_, when you assess a single beta in a multiple regression, you don't need to worry about it's correlation to other features because it's correlation has been partialled out! _In practice_, this obviously is not the case from a computation perspective (condition number), but that's why ridge and SVD comes in handy to modify singular values.

## The Proof

I'm going to cite the proof from *Gregory Gundersen's blog* in his [OLS article](https://gregorygundersen.com/blog/2020/01/04/ols/). 
...
Notice this expression is similar to the normal equation for regressing $\textbf{y}$ on $\textbf{X}_2$ but with $\textbf{M}_1$ in front of $\textbf{y}$ and certain $\textbf{X}_2$ terms. Why not in front of the first $\textbf{X}_2$? Because $\textbf{M}_1$ is an orthogonal projection aka $\textbf{M}=\textbf{M}^2$ and also orthogonal so $\textbf{M}=\textbf{M}^T$. Applying $\textbf{M}_1$ to all the terms cancels out to form the above.

So the residual-on-residual idea of FWL can be expressed in terms of the residual maker matrix $\textbf{M}_1\textbf{y} = \textbf{M}_1\textbf{X}_2\hat{\beta_2} + \textbf{e}$. And that's the derivation.



## Why Is This Useful

But who cares? Why is this useful? Here are some reasons (especially the first):
...
* Viewing `sm.add_constant(X)` to the design matrix for an intercept as partitioned regression where $\textbf{X}_1=\textbf{1}$ which is equivalent to demeaning.

Tree-based ML models (rf, xgb, gbm, cb etc) don't have this neat, elegant interpretation of coefficients! It would be interesting to compare feature importance of a tree-based model and betas of a least squares trained on the same data.
...
And in commodities, there might be lots of chances to use mixed effects models (e.g predict some quantity across countries or different geographical areas). In my last role at a commods hf, I realize I did a project that I could have effectively used mixed effects models to good use.
...
exog = np.random.multivariate_normal(mu, cov, size=50)
endog = beta1 * exog.T[0] + beta2 * exog.T[1] + np.random.normal(0,0.3)

df = pd.DataFrame(np.column_stack((exog, endog)), columns=['face', 'height', 'attr'])

hf = smf.OLS('height ~ face', data=df, hasconst=False).fit()
fh = smf.OLS('face ~ height', data=df, hasconst=False).fit()
ah = smf.OLS('attr ~ height', data=df, hasconst=False).fit()
af = sm.OLS('attr ~ face', data=df hasconst=False).fit()
```


Plotting individual simple regressions of face and height, we see the individual betas are over-estimated because of the correlation between the two.
...
Regressing the orthogonalized (w.r.t face) variables (residuals), demeaned, against each other, we recover the original height beta in the true relationship:
...
For now, the FWL theorem provides us with essential intuition about key facets of multiple regression without going into any implementation details yet. And it's pretty cool that I only discovered it two weeks ago while going through the regression part of ESL and EQI. And RyxCommar, while being a data scientist himself, is highly trusted by quants, so it must be important! 
