...
* **Core Strength Feature**
    ...
    * The January/Structural Breakpoint Problem - Do we evaluate at season start and Jan window separately? Then we assume the good transfer impact is priced into squad values. What if a team doesn't sign anyone in January? But if we generalize that, then why don't we use squad value every 2 months? There is a problem.
